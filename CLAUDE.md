...
- **KostengutspracheViewController** (Swift) — Kostengutsprache KVV 71 form for IBD Gastroenterology; generates PDF, sends via share sheet; pre-fills from Receipt data and prescription scan; resolves medication names via GTIN lookup in AmiKo DB or OCR for pharmacodes; saves all form data back to Receipt on close (patient address, AHV, insurer name, physician hospital/department); includes IBD diagnosis selector (M. Crohn / Colitis ulcerosa); embeds an Indikationscode (IndC) selector between the medication block and the physician block (issue #102) — see `IndCSection.swift`
- **IndCSection.swift** — `IndCEntry` / `IndCParser` / `IndCSectionView` for the BAG Indikationscode (XXXXX.NN). Parses the comma-separated `INDIKATIONSCODE` and newline-separated `INDIKATIONSCODE_TEXT` columns (rust2xml ≥ 3.1.12 / cpp2sqlite `--fhir`), shows a radio list of codes with the matching limitations text, fires a "Kostengutsprache erforderlich" warning when the text contains `kostengutsprache`/`vertrauensarzt`, and falls back to a KVV Art. 71b/c off-label textarea when the product has no IndC. Selection is persisted on `Receipt` (`indikationscode`, `indikationscodeTitle`, `indikationscodeText`, `indikationscodeOffLabel`) and rendered into the KVV 71 PDF as a code/indication/limitations-text quote block. The parser identifies real code starts via `^\d+\.\d+: ` and accumulates non-matching lines as continuation so the KoGu detector sees keywords below the first line — earlier a naïve `\n` split dropped ~72% of KoGu triggers
...
- **Receipt** — imported prescription from `.amk` files; contains Operator, Patient (with healthCardNumber, insurerName, ahvNumber), Products, and IndC fields (`indikationscode`, `indikationscodeTitle`, `indikationscodeText`, `indikationscodeOffLabel`)
